...
Both kube-burner, through https://github.com/kube-burner/kube-burner-ocp/?tab=readme-ov-file#virt-workloads[kube-burner-ocp virtualization related workloads] (virt-density, virt-capacity-benchmark, virt-clone, virt-migration,  etc.) and k8s-netperf, through the `--vm` and `--vm-image` flags, support running workloads on top of OpenShift Virtualization, creating virtual machines at scale allowing operators to evaluate the cluster behavior under high-density conditions.

k8s-io also supports running storage benchmarks on virtual machines through the `kind` property of its config file (`pod` or `vm`).


== Conclusion

Overall, Kubernetes-native benchmarking tools complement traditional system benchmarks by providing visibility into cluster-level performance and cloud-native behaviors. Most of these tools allow indexing results on external ElasticSearch/OpenSearch instances in a cloud-native way and provide support to OpenShift Virtualization workloads. Subsequent chapters in this course will dive deeper into each of these tools, providing practical examples and configuration guidance.
